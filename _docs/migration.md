...
{% callout info %}
**Heads up!** This document is not 100% comprehensive, and will continue to change as Bootstrap 4's beta continues to develop. If you have any specific questions about something not listed here, please reach out to us on [Github]({{ site.github }}) or [Slack]({{ site.slack }}).
{% endcallout %}
...
- Dropped IE8, IE9, and iOS 6 support. Athena only supports the [latest 2 versions of major browsers]({{ "getting-started/browsers-devices/#supported-browsers" | prepend: site.baseurl }}).
- Added official support for Android's Browser and WebView.
...
- Global font-size increased from `14px` to `.875rem` by default, and `1rem` at the `-md` breakpoint and up.
- Added a new grid tier for smaller devices at `576px` and below (the new `xs` tier).
...
- Dropped the Affix jQuery plugin. Use the `.sticky-top` utility class instead.
...
### Misc
- Non-responsive usage of Athena is not supported.
...
New to Bootstrap 4 and Athena is the Reboot, a new stylesheet that builds on Normalize with somewhat opinionated reset styles. Selectors appearing in this file only use elements—there are no classes here. This isolates our reset styles from our component styles for a more modular approach. Some of the most important resets this includes are the `box-sizing: border-box` change, moving from `em` to `rem` units on many elements, link styles, and many form element resets.
...
TODO: audit list of stuff in Bootstrap 3 that was marked as deprecated
