...
In need of a button, but not the hefty background colors they bring? Replace the default modifier classes with the `.btn-outline-*` ones for a stylish bordered button with white background.

Note that a "default" outline button variant is not provided, since `.btn-default` is already styled with an outline.
...
<button type="button" class="btn btn-outline-inverse">Inverse</button>
{% endexample %}

Use the `.btn-outline-i-` outline variants against dark backgrounds for adequate text color contrast.

Note that "secondary" and "inverse" variants aren't provided here.
...
<div class="bg-inverse p-2">
  ...
  <button type="button" class="btn btn-outline-i-danger">Danger</button>
</div>
{% endexample %}
...
Athena's `.button` styles can be applied to other elements, such as `<label>`s, to provide checkbox or radio style button toggling. Add `data-toggle="buttons"` to a `.btn-group` containing those modified buttons to enable toggling in their respective styles.
